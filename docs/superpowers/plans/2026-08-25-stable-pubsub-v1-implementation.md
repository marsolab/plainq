...
At the start of every decoded committed entry, `FSM.Apply` calls `ReplicaApplyGuard.Check`. Once quarantined, later publish and non-publish entries return typed unavailable outcomes without invoking storage or the fatal callback, so an unhealthy voter neither mutates further nor crash-loops while Raft advances. `FSM.Snapshot` checks the same durable latch before `BeginSnapshot`, so a quarantined replica cannot produce or propagate recovery material and only a snapshot pinned by a healthy source may recover it. `FSM.Restore` deliberately does not use this apply gate: it stages and commits the snapshot, verifies `TopicInventory`, and only then calls `Recover`. Tests execute partial → non-publish → publish → rejected snapshot → verified restore and prove only the verified restore reopens mutation.
