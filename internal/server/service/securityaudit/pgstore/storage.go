...
import (
	...
	"github.com/marsolab/plainq/internal/server/service/securityaudit/pgstore/sqlcgen"
)

var _ securityaudit.Storage = (*Storage)(nil)

// Storage persists append-only audit events in PostgreSQL.
type Storage struct {
	queries *sqlcgen.Queries
}

// NewStorage constructs PostgreSQL audit persistence.
func NewStorage(pool *pgxpool.Pool) (*Storage, error) {
	if pool == nil {
		return nil, errors.New("pool is nil")
	}

	return &Storage{queries: sqlcgen.New(pool)}, nil
}
...
func (s *Storage) Append(ctx context.Context, event securityaudit.Event) error {
	if err := securityaudit.AppendTx(ctx, auditAppender{s.queries}, event); err != nil {
		return fmt.Errorf("append postgres security audit: %w", err)
	}
...
func (s *Storage) List(ctx context.Context, query securityaudit.Query) (securityaudit.Page, error) {
	...
	if err != nil {
		return securityaudit.Page{}, fmt.Errorf("list postgres audit events: %w", err)
	}
...
func (a auditAppender) AppendAuditEvent(ctx context.Context, event securityaudit.Event) error {
	...
	if err := a.queries.InsertAuditEvent(ctx, sqlcgen.InsertAuditEventParams{
		...
		return fmt.Errorf("insert postgres audit event: %w", err)
	}

	return nil
}

func postgresEvent(row sqlcgen.SecurityAuditEvent) (securityaudit.Event, error) {
	metadata := map[string]string{}
	if err := json.Unmarshal(row.MetadataJson, &metadata); err != nil {
		return securityaudit.Event{}, fmt.Errorf("decode postgres audit metadata: %w", err)
	}
