...
// ListTopics returns all pub/sub topics and their subscriptions.
func (s *Storage) ListTopics(ctx context.Context) (*queue.ListTopicsResponse, error) {
	...
	defer func() {
		if err := rows.Close(); err != nil {
			s.logger.Error("close pubsub rows", "error", err.Error())
		}
	}()
...
// Subscribe subscribes a queue to a pub/sub topic.
...
// Unsubscribe removes a queue subscription from a pub/sub topic.
...
// Publish fans messages out to every queue subscribed to the topic.
...
func (s *Storage) listSubscriptions(ctx context.Context, topicID string) ([]queue.Subscription, error) {
	rows, err := s.db.QueryContext(ctx, `
		select s.subscription_id, s.topic_id, s.queue_id, coalesce(q.queue_name, ''), s.created_at
...
func (s *Storage) ensureTopicExists(ctx context.Context, topicID string) error {
	var exists int
	err := s.db.QueryRowContext(ctx, `select 1 from topic_properties where topic_id = ?;`, topicID).Scan(&exists)
	if errors.Is(err, sql.ErrNoRows) {
		return fmt.Errorf("topic %q: %w", topicID, errkit.ErrNotFound)
	}
	if err != nil {
		return fmt.Errorf("lookup topic %q: %w", topicID, err)
	}
