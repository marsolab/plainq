...
const (
	topicDeleteLowerBoundQuery = `SELECT COUNT(*),
		COALESCE(SUM(length(CAST(s.subscription_id AS BLOB)) + length(CAST(s.topic_id AS BLOB)) + length(CAST(s.queue_id AS BLOB)) + length(CAST(COALESCE(q.queue_name, '') AS BLOB))), 0),
...
func (s *Storage) PreflightDeleteTopic(ctx context.Context, topicID string, limit int) error {
	...
	if err := preflightDeleteSubscriptions(ctx, tx, topicDeleteLowerBoundQuery, topicDeleteRowsQuery, topicID, limit, pubSubDeleteTopic); err != nil {
...
func (s *Storage) PreflightDeleteQueue(ctx context.Context, input *v1.DeleteQueueRequest, limit int) error {
	if input == nil {
	...
	if err := preflightDeleteSubscriptions(ctx, tx, queueDeleteLowerBoundQuery, queueDeleteRowsQuery, queueID, limit, pubSubDeleteQueue); err != nil {
...
func preflightDeleteSubscriptions(
	...
	return sizer.CheckLimit(limit, true)
}
