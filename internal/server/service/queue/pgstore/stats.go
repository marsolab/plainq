...
//
// Counting is the only way to be right about either number: the delta tracking
// on the write path starts from zero on a process that restarts onto a
// database full of messages, and nothing emits an event when a visibility
// timeout lapses and a claimed message quietly becomes available again.
func (s *Storage) queueStats(ctx context.Context, queueID string) (queueStats, error) {
	var stats queueStats

	row := s.pool.QueryRow(ctx, queryQueueStats(queueID))
