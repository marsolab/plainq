...
const validationErrorFormat = "validation error: %w"
...
func (s *Service) deleteTopicHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err := validateQueueID(topicID); err != nil {
		httpkit.ErrorHTTP(w, r, fmt.Errorf(validationErrorFormat, err))
		return
	}

	if err := s.storage.DeleteTopic(r.Context(), topicID); err != nil {
		httpkit.ErrorHTTP(w, r, err)
		return
	}
...
func (s *Service) unsubscribeTopicHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err := validateQueueID(subscriptionID); err != nil {
		httpkit.ErrorHTTP(w, r, fmt.Errorf(validationErrorFormat, err))
		return
	}

	if err := s.storage.Unsubscribe(r.Context(), topicID, subscriptionID); err != nil {
		httpkit.ErrorHTTP(w, r, err)
		return
	}
