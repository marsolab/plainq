...
func (s *Storage) CreateGrant(ctx context.Context, input agent.CreateGrantInput) (agent.GrantRecord, error) {
	...
	err := s.withinTx(ctx, func(tx pgx.Tx) error {
		...
		replayed, found, err := postgresPolicyReplay[agent.GrantRecord](
		...
		if input.ResourceID != "*" {
			exists, err = queries.PolicyResourceExists(ctx, sqlcgen.PolicyResourceExistsParams{
				ResourceKind: string(input.ResourceKind), TenantID: input.TenantID, ResourceID: input.ResourceID,
			})
			if err != nil {
				return fmt.Errorf("resolve grant resource: %w", err)
			}
		...
		created, err = postgresGrantRecord(row)
		if err != nil {
			return err
		}
...
func (s *Storage) ListGrants(ctx context.Context, input agent.ListGrantsInput) (agent.GrantPage, error) {
	...
	for _, row := range rows {
		record, err := postgresGrantRecord(row)
		if err != nil {
...
func (s *Storage) DeleteGrant(ctx context.Context, input agent.DeleteGrantInput) error {
	return s.withinTx(ctx, func(tx pgx.Tx) error {
		queries := sqlcgen.New(tx)
		policyTx := postgresPolicyTransaction{queries: queries, now: input.Policy.Audit.CreatedAt.UTC()}
...
func postgresGrantRecord(row sqlcgen.AgentResourceGrant) (agent.GrantRecord, error) {
