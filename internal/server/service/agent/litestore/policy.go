...
// sqlitePolicyTransaction adapts the generated queries of an already-open
// registry transaction to the shared quota and audit transaction seams.
type sqlitePolicyTransaction struct {
	queries *sqlcgen.Queries
	now     time.Time
}

func (tx sqlitePolicyTransaction) ReserveRate(
	ctx context.Context,
	tenantID string,
	action authz.Action,
	units uint64,
	...
	storedUnits, err := sqlitePolicyInt64(units, "rate units")
	if err != nil {
...
func (tx sqlitePolicyTransaction) ApplyActualUsage(ctx context.Context, delta quota.UsageDelta) error {
	...
	if delta.ActiveCredentialsRemoved != 0 {
		removed, err := sqlitePolicyInt64(delta.ActiveCredentialsRemoved, "removed credentials")
		if err != nil {
			return err
		}
...
func (tx sqlitePolicyTransaction) AppendAuditEvent(ctx context.Context, event securityaudit.Event) error {
...
func sqlitePolicyReplay[T any](
	...
	var zero T
	if err := validateSQLitePolicy(mutation, expectedAction, expectedTenantID, expectedResourceID); err != nil {
	...
	if err := json.Unmarshal([]byte(row.ResponseJson), &zero); err != nil {
...
func finishSQLitePolicy[T any](
	ctx context.Context,
	tx sqlitePolicyTransaction,
	mutation policytx.Mutation,
	result T,
) error {
	...
	err = tx.queries.InsertPolicyIdempotency(ctx, sqlcgen.InsertPolicyIdempotencyParams{
		...
		ResponseJson: string(response), CreatedAtNs: mutation.Audit.CreatedAt.UnixNano(),
...
func sqlitePolicyInt64(value uint64, field string) (int64, error) {
